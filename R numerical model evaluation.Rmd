...
對於總離差平方和怎麼拆解的，有興趣的我在最下面推導。
則可用比例的概念來思考，如果總變異量為1，那麼迴歸模型解釋的百分比佔多少，殘差又佔多少？
\[1=\frac{SS_{regression}}{SS_{t}}+\frac{SS_{e}}{SS_{t}}\]

回歸可解釋的變異量
\[R^{2}=\frac{SS_{regression}}{SS_{t}}=1-\frac{SS_{e}}{SS_{t}}\]

一般來說，\(R^{2}\)越大，表示模型擬合效果越好。<br> 模型解釋了多少，反映的就是大概有多準，隨著樣本數量的增加，\(R^{2}\)必然增加，無法真正說明準確程度，只能大概定量。
...
```{r prove sst}
#這邊簡單驗證一下
y <- sort(round(runif(10,min=0,max=50)))
x <- 1:10
x_mean <- mean(x)
y_mean <- mean(y)
