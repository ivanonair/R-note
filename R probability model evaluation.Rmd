...
KS值越大，表示模型能夠將正、負分類分開的程度越大。KS值的範圍是[0，1] 。

由於KS值能找出模型中差異最大的一個門檻值，因此適合用來找門檻值cut_off。但是ks值只能反映出哪個門檻值是區分最大的，而不能總體反映出總體模型的分類效果，這點AUC值更能勝任。
如果模型中採用的Feature不佳，沒辦法有效分類樣本，那麼AUC值就會很低，不管門檻值怎麼調都不會好。
KS值表示模型將正樣本和負樣本區分開來的最大分類能力。值越大，模型的預測準確性越好。一般，KS值大於0.2時，即可認為模型預測準確性達到可用要求。一般模型KS值介於0.2到0.6之間，很難超過0.6。

承接ROC圖，這裡繼續使用ROCR包繪圖
```{r ks plot}
perf <- ROCR::performance(pred,"tpr","fpr")
